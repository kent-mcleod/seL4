...
BOOT_CODE void map_kernel_frame(paddr_t paddr, pptr_t vaddr, vm_rights_t vm_rights, vm_attributes_t attributes)
{
    ...
    armKSGlobalKernelPT[GET_PT_INDEX(vaddr, (PT_LEVELS-1))] = pte_4k_page_new(uxn, paddr,
                                                       0, /* global */
                                                       1, /* access flag */
                                                       shareable,
...
BOOT_CODE void map_kernel_window(void)
{
    ...
    for (paddr = PADDR_BASE; paddr < PADDR_TOP; paddr += BIT(seL4_LargePageBits)) {
        armKSGlobalKernelPDs[GET_PUD_INDEX(vaddr)][GET_PT_INDEX(vaddr, (PT_LEVELS-2))] = pte_pte_page_new(
#ifdef CONFIG_ARM_HYPERVISOR_SUPPORT
                                                                              0, // XN
#else
                                                                              1, // UXN
...
static BOOT_CODE void map_it_pt_cap(cap_t vspace_cap, cap_t pt_cap)
{
    ...
    assert(pude_pude_pd_ptr_get_present(pud));
    pd = paddr_to_pptr(pude_pude_pd_ptr_get_pd_base_address(pud));
    *(pd + GET_PT_INDEX(vptr, (PT_LEVELS-2))) = pte_pte_table_new(
                                     pptr_to_paddr(pt)
                                 );
}
...
BOOT_CODE word_t arch_get_n_paging(v_region_t it_v_reg)
{
    return
        ...
        get_n_paging(it_v_reg, GET_LVL_PGSIZE_BITS(PT_LEVELS - 2));
}
...
BOOT_CODE cap_t create_it_address_space(cap_t root_cnode_cap, v_region_t it_v_reg)
{
    ...
    for (vptr = ROUND_DOWN(it_v_reg.start, GET_LVL_PGSIZE_BITS(PT_LEVELS - 2));
         vptr < it_v_reg.end;
         vptr += GET_LVL_PGSIZE(PT_LEVELS - 2)) {
...
static lookupPTSlot_ret_t lookupPDSlot(vspace_root_t *vspace, vptr_t vptr)
{
    ...
    if (!pude_pude_pd_ptr_get_present(pudSlot.pudSlot)) {
        ...
        pte_t *pd;
        pte_t *pdSlot;
        word_t pdIndex = GET_PT_INDEX(vptr, (PT_LEVELS - 2));
...
static lookupPTSlot_ret_t lookupPTSlot(vspace_root_t *vspace, vptr_t vptr)
{
    ...
    if (!pte_pte_table_ptr_get_present(pdSlot.ptSlot)) {
        ...
        pte_t *pt;
        pte_t *ptSlot;
        word_t ptIndex = GET_PT_INDEX(vptr, (PT_LEVELS-1));
...
static lookupFrame_ret_t lookupFrame(vspace_root_t *vspace, vptr_t vptr)
{
    ...
    case pude_pude_pd: {
        ...
        pte_t *pdSlot = pd + GET_PT_INDEX(vptr, (PT_LEVELS - 2));

        if (pte_ptr_get_pte_type(pdSlot) == pte_pte_page) {
            ret.frameBase = pte_pte_page_ptr_get_page_base_address(pdSlot);
        ...
        if (pte_ptr_get_pte_type(pdSlot) == pte_pte_table) {
            ...
            pte_t *ptSlot = pt + GET_PT_INDEX(vptr, (PT_LEVELS-1));

            if (pte_4k_page_ptr_get_present(ptSlot)) {
                ret.frameBase = pte_pte_page_ptr_get_page_base_address(ptSlot);
...
void unmapPageTable(asid_t asid, vptr_t vptr, pte_t *target_pt)
{
    ...
    for (word_t i = 0; i < PT_LEVELS - 1 && pt != target_pt; i++) {
        ptSlot = pt + GET_PT_INDEX(vptr, i);
...
void unmapPage(vm_page_size_t page_size, asid_t asid, vptr_t vptr, pptr_t pptr)
{
    ...
    case ARMSmallPage: {
        ...
        if (pte_4k_page_ptr_get_present(lu_ret.ptSlot) &&
            pte_pte_page_ptr_get_page_base_address(lu_ret.ptSlot) == addr) {
                                                ...
                                                cte_t *cte, cap_t cap, word_t *buffer)
{
    ...
    vaddr = getSyscallArg(0, buffer) & (~MASK(GET_LVL_PGSIZE_BITS(PT_LEVELS - 2)));
    vspaceRootCap = current_extra_caps.excaprefs[0]->cap;
