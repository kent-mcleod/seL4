...
def generate_param_list(input_params, output_params):
    ...
    for param in input_params:
        if not param.type.pass_by_reference():
            params.append(rust_render_parameter_name(param.type, param.name))
        else:
            params.append(rust_render_parameter_name(param.type.pointer(), param.name, mutable_ref=False))
    for param in output_params:
        if param.type.pass_by_reference():
            params.append(rust_render_parameter_name(param.type.pointer(), param.name, mutable_ref=True))
...
def generate_marshal_expressions(params, num_mrs, structs, wordsize):
    ...
    def generate_param_code(param, first_bit, num_bits, word_array, wordsize):
        ...
        for i in range(num_words):
            expr = rust_expression(param.type, param.name, i, syscall_stub_gen.struct_members(param.type, structs))
                ...
                if param.type.double_word:
                    ...
                    for word in words:
                        if type(param.type) is syscall_stub_gen.StructType:
                            result.append("\tresult.%s = %s { words: [%s] };" % (param.name, param.type.name, word % source_words))
                        else:
                            result.append("\tresult.%s = %s as %s;" % (param.name, word % source_words, translate_type(param.type.name)))
